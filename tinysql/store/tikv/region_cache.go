...
func (c *RegionCache) GroupKeysByRegion(bo *Backoffer, keys [][]byte, filter func(key, regionStartKey []byte) bool) (map[RegionVerID][][]byte, RegionVerID, error) {
	...
	groupBy := make(map[RegionVerID][][]byte)
	var firstRegion RegionVerID
	var location *KeyLocation
	var err error
	for index, key := range keys {
		if location == nil || !location.Contains(key) {
			location, err = c.LocateKey(bo, key)
			if err != nil {
				return nil, firstRegion, err
			}
			if filter != nil && filter(key, location.StartKey) {
				continue
			}
		}
		keyRegion := location.Region
		if index == 0 {
			firstRegion = keyRegion
		}
		groupBy[keyRegion] = append(groupBy[keyRegion], key)
	}
	return groupBy, firstRegion, nil
}
