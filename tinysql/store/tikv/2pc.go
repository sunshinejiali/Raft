...
func (c *twoPhaseCommitter) buildPrewriteRequest(batch batchKeys) *tikvrpc.Request {
	...
	for _, key := range batch.keys {
		mutationEX := c.mutations[string(key)]
		mutations = append(mutations, &mutationEX.Mutation)
	}
...
func (actionCommit) handleSingleBatch(c *twoPhaseCommitter, bo *Backoffer, batch batchKeys) error {
	...
	// YOUR CODE HERE (lab3).
	// panic("YOUR CODE HERE")
	regionErr, errRegion := resp.GetRegionError()
	if errRegion != nil {
		return errors.Trace(errRegion)
	}
	if regionErr != nil {
		err = bo.Backoff(BoRegionMiss, errors.New(regionErr.String()))
		if err != nil {
	...
	keyError := commitResp.GetError()
	if keyError != nil {
		c.mu.RLock()
		defer c.mu.RUnlock()
		err = extractKeyErr(keyError)
		if c.mu.committed {
			return errors.Trace(err)
		}
		return err
...
func (actionCleanup) handleSingleBatch(c *twoPhaseCommitter, bo *Backoffer, batch batchKeys) error {
	...
	keyErr := resp.Resp.(*pb.BatchRollbackResponse).GetError()
	if keyErr != nil {
	...
	defer func() {
		...
		if !committed && !undetermined {
			...
			go func() {
				...
				// YOUR CODE HERE (lab3).
				// panic("YOUR CODE HERE")
				_ = c.cleanupKeys(NewBackoffer(cleanupKeysCtx, cleanupMaxBackoff).WithVars(c.txn.vars), c.keys)
				c.cleanWg.Done()
			}()
		}
