...
func (lr *LockResolver) resolveLock(bo *Backoffer, l *Lock, status TxnStatus, cleanRegions map[RegionVerID]struct{}) error {
	...
	for {
		...
		// YOUR CODE HERE (lab3).
		// panic("YOUR CODE HERE")
		resolveLockRequest := &kvrpcpb.ResolveLockRequest{
			StartVersion:         l.TxnID,
		}
		if status.IsCommitted() {
			resolveLockRequest.CommitVersion = status.CommitTS()
		}
		req = tikvrpc.NewRequest(tikvrpc.CmdResolveLock, resolveLockRequest)
