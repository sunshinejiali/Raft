...
func (s *tikvSnapshot) get(bo *Backoffer, k kv.Key) ([]byte, error) {
	...
	for {
		...
		if keyErr := cmdGetResp.GetError(); keyErr != nil {
			...
			lock, errLock := extractLockFromKeyErr(keyErr)
			if errLock != nil {
				return nil, errors.Trace(errLock)
			}
			locks := []*Lock{lock}
			msBeforeTxnExpired, errResolveLock := cli.ResolveLocks(bo, s.version.Ver, locks)
			if errResolveLock != nil {
				return nil, errors.Trace(errResolveLock)
			}
