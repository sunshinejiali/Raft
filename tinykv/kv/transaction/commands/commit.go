...
func (c *Commit) PrepareWrites(txn *mvcc.MvccTxn) (interface{}, error) {
	...
	if commitTs <= txn.StartTS {
		return nil, errors.New("The commitTS isn't greater than the transaction startTs.")
	}
...
func commitKey(key []byte, commitTs uint64, txn *mvcc.MvccTxn, response interface{}) (interface{}, error) {
	...
	if lock == nil || lock.Ts != txn.StartTS {
		...
		write, _, writeError := txn.CurrentWrite(key)
		if writeError != nil {
		...
		if write != nil {
			if write.Kind == mvcc.WriteKindRollback {
				...
				keyError := &kvrpcpb.KeyError{Abort: fmt.Sprintf("need to be aborted for key %v", key)}
